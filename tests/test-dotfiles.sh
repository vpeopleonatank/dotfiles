...
mkdir -p "$NVM_HOME/.config/nvm/bin" "$NVM_HOME/.config/nvm/versions/node/v22.18.0/bin" "$NVM_HOME/.config/nvm/alias"
printf 'v22.18.0\n' > "$NVM_HOME/.config/nvm/alias/default"
ln -s "$NVM_HOME/.config/nvm/bin/node" "$NVM_HOME/.config/nvm/versions/node/v22.18.0/bin/node"
printf '%s\n' \
...
NVM_DIR="$NVM_HOME/.config/nvm" NVM_LOAD_LOG="$NVM_HOME/nvm-loaded" HOME="$NVM_HOME" XDG_CONFIG_HOME="$NVM_HOME/.config" PATH="$MINIMAL_PATH" \
  zsh -f -i -c "source '$ROOT/zsh/config.zsh'; [[ \"\$(command -v node)\" == \"$NVM_HOME/.config/nvm/versions/node/v22.18.0/bin/node\" ]] && [[ ! -e \"\$NVM_LOAD_LOG\" ]] && [[ \"\$(node --version)\" == 'node:--version' ]] && [[ -e \"\$NVM_LOAD_LOG\" ]] && [[ \"\$(nvm current)\" == 'nvm:current' ]]"
