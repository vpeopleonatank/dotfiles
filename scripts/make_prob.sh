...
for filepath in "$@"; do
    ...
    # Fix template file name as template.cpp
    mv $filepath/template.cpp $filepath/$PROBLEM_NAME.cpp
    # replace problem_name to $PROBLEM_NAME
    REPLACE_REGEX="s/problem_name/${PROBLEM_NAME}/g"
    sed -i $REPLACE_REGEX $filepath/Makefile
    echo $REPLACE_REGEX
